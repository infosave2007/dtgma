...
Existing approaches to mitigating catastrophic forgetting include regularization-based methods such as Elastic Weight Consolidation (EWC) \cite{kirkpatrick2017}, knowledge distillation approaches such as Learning without Forgetting (LwF) \cite{li2016}, replay buffers \cite{buzzega2020}, and parameter isolation techniques including Progressive Neural Networks \cite{rusu2016}, Adapters \cite{houlsby2019}, and LoRA \cite{hu2021}. While effective in certain scenarios, these methods do not provide strict architectural guarantees against task interference.
...
\textbf{Architecture:} 2-layer DTG-MA with 256 hidden dimensions. Input projection: $d_{in} \rightarrow 256$, hidden layer: $256 \rightarrow 256$, output heads: $256 \rightarrow C_t$ (task-specific, one per task). For fair comparison, all methods use 	extbf{task-specific output heads} in our experiments, allowing each task to maintain its own classifier. This is standard for task-incremental learning where task identity is known at inference.
...
\item \textbf{Adapter} \cite{houlsby2019}: Task-specific adapter modules (bottleneck dim 64).
\end{itemize}
...
\textbf{Note on baseline performance.} While all methods use task-specific output heads, the shared encoder remains the main source of interference for regularization-based methods (EWC, Fine-tuning) and replay-based methods (DER++). DTG-MA's attention masking prevents interference at the encoder level as well, which explains the performance gap even with identical output heads.
...
\newblock In \emph{International Conference on Machine Learning (ICML)}, volume 97, pages 2790--2799. PMLR, 2019.
\newblock arXiv:1902.00751
